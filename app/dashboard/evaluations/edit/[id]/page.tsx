...
import { getEvaluationById, updateEvaluation, getCourses } from '@/app/actions'
...
export default function EditEvaluation({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      const [courses, evaluation] = await Promise.all([
        getCourses(),
      ...
      if (evaluation) {
        ...
        setDuration(evaluation.duration)
        setSelectedCourses(evaluation.courses.map(c => c.courseName))
        setAllowMultipleAttempts(evaluation.allowMultipleAttempts ?? false)
        setShowScoreAtEnd(evaluation.showScoreAtEnd ?? true)
        setShowCorrectAnswers(evaluation.showCorrectAnswers ?? false)
        setShuffleQuestions(evaluation.shuffleQuestions ?? false)
        setQuestions((evaluation.questions as unknown as Question[]).map((q: Question) => ({ ...q, points: q.points ?? 1 })))
      } else {
  ...
  const addQuestion = () => {
    setQuestions([...questions, { type: 'MULTIPLE_CHOICE', content: '', correctAnswer: '', points: 1 }])
  }
      ...
      <main className="dashboard-content">
        <header style={{ marginBottom: '2rem' }}>
          <div style={{ fontSize: '0.8rem', color: 'var(--secondary)', marginBottom: '0.5rem' }}>
            <Link href="/dashboard" style={{ color: 'inherit', textDecoration: 'none' }}>Dashboard</Link> / Editar Evaluación
          </div>
          <h1 style={{ margin: 0 }}>Editar Evaluación</h1>
        </header>
        ...
        <form onSubmit={handleSubmit} className="section-card">
          ...
          {questions.map((q, index) => (
            <div key={index} style={{ marginBottom: '2rem', padding: '1.5rem', background: 'var(--accent)', borderRadius: '12px', position: 'relative' }}>
              ...
              <div style={{ display: 'flex', gap: '1rem', marginBottom: '1rem' }}>
                 <div style={{ flex: 1 }}>
                  ...
                  <select 
                    className="form-control" 
                    value={q.type} 
                    onChange={e => updateQuestionField(index, 'type', e.target.value as Question['type'])}
                  >
                ...
                <div style={{ width: '100px' }}>
                  ...
                  <input 
                    ...
                    onChange={e => {
                      ...
                      updateQuestionField(index, 'points', val)
                    }}
                    required 
                  />
                </div>
                <div style={{ flex: 3 }}>
                  ...
                  <input 
                    ...
                    className="form-control" 
                    value={q.content} 
                    onChange={e => updateQuestionField(index, 'content', e.target.value)} 
                    required 
                  />
                </div>
              </div>
              ...
              {q.type === 'MULTIPLE_CHOICE' && (
                <div className="form-group">
                  <label>Opciones (comas)</label>
                  <input 
                    type="text" 
                    className="form-control" 
                    value={q.options || ''} 
                    onChange={e => updateQuestionField(index, 'options', e.target.value)} 
                  />
                </div>
              )}
          ...
          <div style={{ display: 'flex', gap: '1rem', marginTop: '0rem' }}>
            ...
            <Link href="/dashboard" className="course-btn" style={{ textDecoration: 'none', textAlign: 'center', flex: 1 }}>
              Cancelar
            </Link>
          </div>
        </form>
